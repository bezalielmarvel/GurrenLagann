...
def trouverObjet3D(arene, x, y, z):
    """Cette fonctions permet de savoir si un point de coordonnées x, y, z appartient a un objet 3D"""
    ...
    for i in listeObjet3D:
        listeSommets = listeObjet3D[a].getSommets()
        """On recupere tous les sommets de l'objet numero a """

        listeCoord = listeSommets[0].getCoord()
        ...
        if (((x >= min_x) and (x <= max_x)) and ((y >= min_y) and (y <= max_y)) and ((z >= min_z) and (z <= max_z))):
            print("Le point de coordonnées {}, {}, {} appartient à un objet 3D ".format(x, y, z, ))
        else:
            print("Le point de coordonnées {}, {}, {} n'appartient pas à un objet 3D ".format(x, y, z, ))
        a = a + 1